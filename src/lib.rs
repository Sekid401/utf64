...
//!
//! A hierarchical, nibble-based Unicode encoding optimized for English-language text compression.
//!
//! Specification v0.1 — Rust Reference Implementation
//!
//! # Note
//! Rust `char` is a Unicode scalar value (0x0000–0xD7FF, 0xE000–0x10FFFF).
...
/// assert_eq!(encoded[0], b'&'); // Magic byte
/// ```
...
/// Encode a string to UTF-64 and return as a hex string.
pub fn encode_to_hex(s: &str) -> String {
    encode(s).iter().map(|b| format!("{:02x}", b)).collect::<Vec<_>>().join(" ")
}

/// Calculate the size of the encoded payload in bytes (excluding magic).
